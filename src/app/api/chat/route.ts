...
Если пользователь хочет оставить заявку или обсудить проект — предложи оставить контакт (Telegram/телефон) или написать в @overcode.
...
If the user wants to leave a request or discuss a project — offer to leave a contact (Telegram/phone) or write to @overcode.
    ...
    if (!apiKey) {
      ...
      if (isEn) {
        if (lastMsg.includes('pric') || lastMsg.includes('cost') || lastMsg.includes('how much') || lastMsg.includes('rate')) {
          ...
          reply = 'Great question! To give you an accurate answer, let us discuss the details. Leave your Telegram or phone number — we will get in touch within an hour. Or write to @overcode.';
        }
      } else {
        if (lastMsg.includes('цен') || lastMsg.includes('стои') || lastMsg.includes('скольк') || lastMsg.includes('прайс')) {
          ...
          reply = 'Интересный вопрос! Чтобы дать точный ответ, предлагаю обсудить детали. Оставьте ваш Telegram или телефон — свяжемся в течение часа. Или напишите в @overcode.';
        }
      }
